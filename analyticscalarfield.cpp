...
AnalyticScalarField::AnalyticScalarField(const Noise* noise, double a, double f,
                                         QVector<double>& off, int lvl) :
    noise(noise), m_A(a), m_F(f), m_offsets(off), m_levels(lvl)
{}
...
double AnalyticScalarField::value(const Vec2& v) const
{
    ...
    for(int i = 0; i < m_levels; ++i){
        Vec2 vi(v.x*(m_F*pow2) /*+ m_offsets[i] * 0.9*/,
                v.y*(m_F*pow2) /*+ m_offsets[i] * 1.7*/);
